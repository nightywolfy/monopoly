    ...
    try {
      ...
      for (const fullCmd of commands) {
        ...
        switch(cmd) {
          ...
          case '!map': {
            const num = parseInt(args[0], 10);
            if (isNaN(num) || num < 1) { safeSay(defaultTarget, 'Usage: !map <number> (e.g., !map 1, !map 2)'); break; }
          ...
          case '!sound': {
            ...
            if(!file) break;
            if (target === 'player2bot') { io.emit('play-sound', { file }); console.log(`[Sound IRC] ${nick} triggered: ${file}`); }
            else console.log(`[Sound IRC] ${nick} tried to trigger !sound but not PM to player1bot`);
            break;
          }
          default: break;
        }
...
const bots={
  ...
  player2bot:createBot('player2bot','##rento',{delay:10000}),
  player3bot:createBot('player3bot','##rento',{delay:20000}),
  player4bot:createBot('player4bot','##rento',{delay:30000}),
