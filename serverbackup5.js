...
function initializeDefaults(){PLAYERS.forEach(p=>{if(money[p]===undefined)money[p]=10});if(!buildings||typeof buildings!=='object')buildings={};PLAYERS.forEach(p=>{if(!pieces[p])pieces[p]={x:825,y:755}});if(!display1.text)display1.text="";if(!display2.text)display2.text="";if(!labels||typeof labels!=='object')labels={};PLAYERS.forEach(p=>{if(!labels[p])labels[p]=p});saveMoney();saveBuildings();savePieces();saveDisplay1();saveDisplay2();saveDots();saveLabels()}
initializeDefaults();

function safeEmit(event,data){try{io.emit(event,data)}catch(err){console.error(`[Socket] Emit failed (${event}):`,err)}}
function updatePiece(player,x,y){if(!colorMap[player])return;const current=pieces[player];if(current&&current.x===x&&current.y===y)return;pieces[player]={x,y};savePieces();safeEmit('piecesUpdate',pieces)}
...
function setBuilding(space,type,unset=false){const key=String(Number(space));if(Number.isNaN(Number(key)))return false;if(unset){if(!buildings[key])return false;const old=buildings[key];delete buildings[key];saveBuildings();safeEmit('buildingsUpdate',buildings);safeEmit('building-removed',{space:Number(key),type:old});return true}else{const old=buildings[key];if(old===type)return false;if(old)delete buildings[key];buildings[key]=type;saveBuildings();safeEmit('buildingsUpdate',buildings);safeEmit('building-set',{space:Number(key),type});return{removed:old||null,set:type}}}
function bulkUpdateBuildings(spaces,type=null,unset=false){let changed=false;const sanitized=spaces.map(s=>Number(s)).filter(n=>!Number.isNaN(n)&&n>=0&&n<=63);sanitized.forEach(space=>{const key=String(space);if(unset){if(buildings[key]){delete buildings[key];changed=true}}else if(type&&buildings[key]!==type){buildings[key]=type;changed=true}});if(!changed)return false;saveBuildings();safeEmit('buildingsUpdate',buildings);return true}
function clearAllBuildings(){const count=Object.keys(buildings).length;buildings={};saveBuildings();safeEmit('buildingsUpdate',buildings);return count}
function updateDot(num,color){const n=Number(num);if(Number.isNaN(n))return false;const table=currentMap===1?coordinates1:coordinates2,pos=table[n];if(!pos)return false;activeDots[String(n)]={x:pos.x,y:pos.y,color:String(color)};saveDots();safeEmit('draw-dot',{x:pos.x,y:pos.y,color,num:n});return true}
...
function createBot(nick,defaultTarget,options={}){
    ...
    function startSendLoop(){
        ...
        sendInterval=setInterval(()=>{
            ...
            try{client.say(target,msg)}
            catch(err){console.error(`[${nick}] send error:`,err)}
        },SEND_INTERVAL_MS);
    }
    ...
    client.on('message',event=>{
        try{
            ...
            for(const fullCmd of commandsRaw){
                ...
                switch(cmd){
                    case '!set':{
                        if(args[0]?.toLowerCase()!=='all'||args.length!==PLAYERS.length+1){
                            safeSay(defaultTarget,`Usage: !set all <amounts for ${PLAYERS.length} players>`);
                            break;
                        }
                    ...
                    case '!display':{
                        if(args.length!==2||!/^p[1-6]$/i.test(args[0])){
                            safeSay(defaultTarget,'Usage: !display p1-p6 <name>');
                            break;
                        }
                    ...
                    case '!mv':{
                        ...
                        if(target?.toLowerCase()!=='all'){
                            safeSay(defaultTarget,'Only !mv all ... allowed');
                            break;
                        }
                    ...
                    case '!hotel':{
                        ...
                        const spaces=args;
                        if(spaces.length===0){
                            safeSay(defaultTarget,`Usage: ${cmd} <space>`);
                            break;
                        }
                    ...
                    case '!sound':{
                        const file=args[0];
                        if(file)commands.playSound(file);
                        break;
                    }
            ...
            console.error(`Command error ${event.nick}:`,err?.stack||err);
        }
    });
    ...
    return{
        ...
        destroy:()=>{
            ...
            stopSendLoop();
            try{client.quit('shutdown',true)}catch{}
        },
        getState:()=>({nick,isConnected,reconnectDelay})
    };
}

const bots={player1bot:createBot('player1bot','##rento',{delay:30000})};

app.post('/send-irc',(req,res)=>{try{const{bot,target,msg}=req.body||{};if(!msg)return res.status(400).send('Missing message');const cleanMsg=String(msg).trim();if(!cleanMsg)return res.redirect('/');if(!bot)return res.status(400).send('Missing bot');if(!bots[bot])return res.status(400).send('Unknown bot');const finalTarget=typeof target==='string'&&target.trim()?target.trim():'##rento';bots[bot].say(finalTarget,cleanMsg);return res.send('OK')}catch(err){console.error('/send-irc error:',err);return res.status(500).send('Server error')}})
