...
class MonopolyBot(SingleServerIRCBot):
    ...
    def handle_command(self, who, body):
        ...
        if body.lower()=="!up":
            if not self.players:return"No game in progress. Cannot update."
            pos=[];money=[];props=[];houses=[]
            for i in range(1,5):
                p=f"p{i}"
                if p in self.players:
                    pos+=[str(self.players[p]["pos"])];money+=[str(self.players[p]["money"])]
                ...
                else:pos+=["0"];money+=["0"];props+=["0"];houses+=["0"]
            try:
            ...
            except Exception as e:return f"UP error: {e}"
        
        if m:=re.match(r"!alias\s+(\w+)\s+(\w+)",body):
            pl,a=m.groups(); pl=pl.lower(); a=a.lower()
            if not hasattr(self,"aliases"): self.aliases={"p1":set(),"p2":set(),"p3":set(),"p4":set()}
        ...
        if m:=re.match(r"!unalias\s+(\w+)",body):
            a=m.group(1).lower()
            if not hasattr(self,"aliases"): self.aliases={}
    ...
    def _handle_dice_command(self, c, d, p, nick):
        ...
        with self.dice_lock:
            ...
            if d == "dice4":
                self.dice4_streak[pl_key] = self.dice4_streak.get(pl_key, 0) + 1
            else:
