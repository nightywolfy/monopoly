...
class MonopolyBot(SingleServerIRCBot):
    ...
    def handle_command(self, who, body):
        ...
        if body.lower() == "!propertylist":
            try:
                ...
                for delay,pos in enumerate(sorted(self.active_board)):
                    ...
                    threading.Timer(delay * 2,lambda p=pos,c=color:self.connection.privmsg("juntao",f"!dot {p} {c}")).start()
        ...
        if m:
            ...
            self.current_auction = {"pos":pos,"bids":{},"last_bidder":None,"active_players":set(self.players.keys())}
        ...
        if m and self.current_auction:
            ...
            if player_key not in auc["active_players"]:
                return f"{player_key} has folded and cannot bid."
            if auc["bids"]:
                highest_bidder = max(auc["bids"], key=auc["bids"].get)
                if player_key == highest_bidder:
                    return f"{player_key} already has the highest bid."
            highest = max(auc["bids"].values(), default=0)
            new_bid = highest + inc
            if self.players[player_key]["money"] < new_bid:
                return f"Not enough money for this bid ({new_bid})"
            auc["bids"][player_key] = new_bid
            auc["last_bidder"] = player_key
            prop = self.active_board.get(auc["pos"], f"Position {auc['pos']}")
            msg = f"{player_key} bids {new_bid} on {prop}"
            try:
        ...
        if body.lower() == "!fold" and self.current_auction:
            ...
            if not player_key: return f"{caller} is not a recognized player."
            if player_key not in auc["active_players"]:    
                return f"{player_key} already folded."
            highest_bidder = None
            if auc["bids"]:
                highest_bidder = max(auc["bids"], key=auc["bids"].get)
            if player_key == highest_bidder:
                return f"{player_key} is currently the highest bidder and cannot fold."
            ...
            if len(auc["active_players"]) == 1:
                winner = list(auc["active_players"])[0]
                ...
                self.active_board[pos] = f"{winner}-{raw[2:] if raw.startswith('x-') else raw}"
        ...
        if owner and owner!=p:
            if new in self.mortgaged: msg+=" | Property is mortgaged, no rent"
                ...
                if new in (4,13,27,38):
                    ...
                    mult={1:5,2:10,3:15,4:30}.get(len(u),5); rent=abs(sp)*mult
                    self.players[p]["money"]-=rent; self.players[owner]["money"]+=rent; play_rent_sound(); msg+=f" | Utility {sp}*{mult}={rent}"
                else:
