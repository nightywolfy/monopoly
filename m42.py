...
class MonopolyBot:
    ...
    def send_chat(self,target,text):
        ...
        try:
            if not self.sio.connected:
                print("[WS] send_chat skipped: socket not connected", flush=True)
                return
            ...
            print(f'[WS] send_chat error: {e}', flush=True)
    def _setup_ws_handlers(self):
        ...
        def connect_error(data):
            print(f"[WS] Connect error: {data}", flush=True)
            ...
            try:
                if self.sio.connected:
                    try:
                        ...
                        print(f"[WS] disconnect before reconnect error: {e}", flush=True)
                self.sio.connect(self.ws_url, wait_timeout=10, transports=['websocket'])
                self._last_ws_activity = time.time()
                self.sio.wait()
            except Exception as e:
    ...
    def _handle_ws_command(self,data):
        ...
        try:
            ...
            print(f"[WS] Command error: {e}", flush=True)
    ...
    def _process_msg_queue(self):
        while True:
            ...
            except Exception as e: print(f"[WS] send_chat queue error: {e}", flush=True)
    ...
    def auto_up(self):
        ...
        try:
            ...
            print(f"[WS] auto_up error: {e}", flush=True)
...
if __name__=="__main__":
    ...
    print("[MAIN] Bot started, entering watchdog loop", flush=True)
    try:
        while not _shutdown.is_set():
            if not bot.ws_thread.is_alive() or not bot.sio.connected:
                print("[MAIN] WARNING: ws not connected/thread dead, restarting", flush=True)
                try:
                    if bot.sio.connected:
                        bot.sio.disconnect()
                except Exception as e:
                    print(f"[MAIN] disconnect during restart error: {e}", flush=True)
                if not bot.ws_thread.is_alive():
                    bot.ws_thread = threading.Thread(target=bot._run_ws_client, daemon=True)
                    bot.ws_thread.start()
            if not bot.msg_worker.is_alive():
                print("[MAIN] WARNING: msg_worker is dead, restarting it", flush=True)
                bot.msg_worker = threading.Thread(target=bot._process_msg_queue, daemon=True)
                bot.msg_worker.start()
            _shutdown.wait(30)
