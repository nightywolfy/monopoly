...
if(!spaces.length){safeSay(defaultTarget,'Usage: !house1 <space>');break;}
bulkUpdateBuildings(spaces,'house1',false);
break;
}
case '!house2':{
const spaces=args.map(a=>parseInt(a,10)).filter(n=>!isNaN(n)&&n>=0&&n<=63);
if(!spaces.length){safeSay(defaultTarget,'Usage: !house2 <space>');break;}
bulkUpdateBuildings(spaces,'house2',false);
break;
}
case '!house3':{
const spaces=args.map(a=>parseInt(a,10)).filter(n=>!isNaN(n)&&n>=0&&n<=63);
if(!spaces.length){safeSay(defaultTarget,'Usage: !house3 <space>');break;}
bulkUpdateBuildings(spaces,'house3',false);
break;
}
case '!house4':{
const spaces=args.map(a=>parseInt(a,10)).filter(n=>!isNaN(n)&&n>=0&&n<=63);
if(!spaces.length){safeSay(defaultTarget,'Usage: !house4 <space>');break;}
bulkUpdateBuildings(spaces,'house4',false);
break;
}
case '!hotel':{
const spaces=args.map(a=>parseInt(a,10)).filter(n=>!isNaN(n)&&n>=0&&n<=63);
if(!spaces.length){safeSay(defaultTarget,'Usage: !hotel <space>');break;}
bulkUpdateBuildings(spaces,'hotel',false);
break;
}
case '!clearall':{
clearAllBuildings();
...
args.slice(0,-1).forEach(n=>updateDot(n,color));
}
break;
}
case '!cleardot':{
...
socket.on('cmd-set-building',({space,type})=>{if(typeof space==='undefined'||!type)return;setBuilding(space,type,false);});
socket.on('cmd-remove-building',space=>{if(typeof space==='undefined')return;setBuilding(space,null,true);});
...
try{
saveMoney();saveBuildings();savePieces();saveDisplay1();saveDisplay2();saveDots();
for(const bot of Object.values(bots)){try{bot.destroy();}catch(err){console.error(`[Server] Error destroying bot:`,err);}}
...
server.listen(PORT, () => console.log(`[Server] Running at http://192.168.1.66:${PORT}`));
