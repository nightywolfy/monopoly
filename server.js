...
function bulkUpdateBuildings(spaces,type=null,unset=false){
    ...
    saveBuildings();safeEmit('buildingsUpdate',buildings);
    return true;
}

function clearAllBuildings(){
    const count=Object.keys(buildings).length;
    buildings={};saveBuildings();safeEmit('buildingsUpdate',buildings);
    return count;
}
...
function removeDot(num){
    ...
    delete activeDots[String(n)];saveDots();safeEmit('remove-dot',n);
    return true;
}
...
const bots={
    player1bot:createBot('player3bot','##rento',{delay:25000}),
    player2bot:createBot('player4bot','##rento',{delay:50000})
};
