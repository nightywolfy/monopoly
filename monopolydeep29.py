...
class MonopolyBot(SingleServerIRCBot):
    def __init__(self, ch, nick, server, port, users=None, num_players=4):
        ...
        self.house_costs = {"red":200,"orange":200,"yellow":150,"green":100,"blue":100,"pink":150,"brown":50,"dblue":50,"white":60,"purple":100,"aqua":140,"black":180}
    ...
    def handle_channel_message(self,c,nick,msg):
        ...
        if msg.lower().startswith(("!mortgage","!redeem")):
            self.auto_up()
        if r: 
            if msg.lower().startswith(("!start","!move","!add","!teleport","!addonehouse","!removeonehouse","!remove")):
                self.auto_up()
    ...
    def handle_command(self, who, body):
        ...
        if body.lower()=="!up":
            if not self.players:
                return "No game in progress. Cannot update."
            self.auto_up()
            return ""
        ...
        if m:=re.match(r"!insert\s+(p\d+)\s+(-?\d+)",body):
                ...
                self.players[pl]={"money":amt,"pos":0};self.aliases.setdefault(pl,set()).add(pl)
                try:self.num_players=max(self.num_players,int(pl[1:]))
                except:pass
                self.jailed[pl]=False;self.auto_up()
                return f"{pl} inserted with ${amt} at position 0"
        ...
        if m and self.current_auction:
            ...
            auc["bid_timer"]=threading.Timer(20,auto_win)
        ...
        if body.lower()=="!fold" and self.current_auction:
            ...
            if player_key==highest_bidder:
                return f"{player_key} is currently highest bidder and cannot fold"
        ...
        if owner and owner!=p:
            if new in self.mortgaged: msg+=" | Property is mortgaged, no rent"
                ...
                if new in (4,13,27,38):
                    ...
                    mult={1:4,2:8,3:16,4:32}.get(len(u),4); rent=abs(sp)*mult
                    ...
                    mult={1:4,2:8,3:16,4:32}.get(len(u),5); rent=abs(sp)*mult
                    self.players[p]["money"]-=rent; self.players[owner]["money"]+=rent; play_rent_sound(); msg+=f" | Utility {sp}*{mult}={rent}"
                else:
