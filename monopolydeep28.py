...
class MonopolyBot(SingleServerIRCBot):
    ...
    def handle_channel_message(self,c,nick,msg):
        ...
        if r: 
             if msg.lower().startswith(("!start","!move","!add","!teleport","!switch","!addonehouse","!removeonehouse","!remove")): self.auto_up()
    ...
    def handle_command(self, who, body):
        ...
        if m:
            ...
            self.current_auction={"pos":pos,"bids":{},"last_bidder":None,"active_players":set(self.players.keys()),"bid_timer":None}
        ...
        if m and self.current_auction:
            ...
            if player_key not in auc["active_players"]:return f"{player_key} has folded and cannot bid."
            if auc["bids"] and player_key==max(auc["bids"],key=auc["bids"].get):return f"{player_key} already has the highest bid."
            ...
            auc["bid_timer"]=threading.Timer(30,auto_win)
        ...
        if body.lower()=="!fold" and self.current_auction:
            ...
            player_key=next((k for k,s in self.aliases.items() if caller in s or caller==k),None)
            if not player_key:return f"{caller} is not a recognized player."
            if player_key not in auc["active_players"]:return f"{player_key} already folded."
            highest_bidder=max(auc["bids"],key=auc["bids"].get) if auc["bids"] else None
        ...
        if body.lower()=="!resetauction":
            ...
            if auc.get("bid_timer"):auc["bid_timer"].cancel()
            pos=auc["pos"]
            raw=self.active_board.get(pos,"")
            if raw.startswith("x-"):
                self.active_board[pos]=raw[2:]
            auc["bids"]={}
            auc["last_bidder"]=None
            auc["active_players"]=set(self.players.keys())
        ...
        if owner and owner!=p:
            if new in self.mortgaged: msg+=" | Property is mortgaged, no rent"
                ...
                if new in (4,13,27,38):
                    ...
                    mult={1:5,2:10,3:20,4:40}.get(len(u),5); rent=abs(sp)*mult
                    self.players[p]["money"]-=rent; self.players[owner]["money"]+=rent; play_rent_sound(); msg+=f" | Utility {sp}*{mult}={rent}"
                else:
