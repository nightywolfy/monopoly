...
class MonopolyBot(SingleServerIRCBot):
    ...
    def handle_channel_message(self,c,nick,msg):
        success,r=self.handle_command(nick,msg)
        if r:
            if success and msg.lower().startswith(("!start","!move","!add","!teleport","!addonehouse","!removeonehouse","!remove","!freeloan","!gobonus","!jailpay","!switch")):self.auto_up()
    ...
    def handle_command(self,who,body):
        ...
        if m:=re.match(r"!start\s+(\d+)\s*(deep|regular)?",body):
            ...
            return True, f"Game started with {n} players ({mode})"
        ...
        if m:
            ...
            if curr + len(props) > max_h: return False, f"Cannot add houses: {color} set would exceed max ({max_h}) houses"
            cost = self.house_costs.get(color,0) * len(props)
            if self.players[owner]['money'] < cost: return False, f"{owner} does not have enough money to buy houses ({cost} required)"
            ...
            return True, f"Added 1 house to each property in {color} set. {owner} charged {cost}"
        ...
        if m:
            ...
            for x in props:
                if self.houses.get(x,0) <= 0: return False, f"Cannot remove house: {x} in {color} set already has 0 houses"
            ...
            self.connection.privmsg("player2bot", f"!sound {sound_file}")
            return True, f"Removed 1 house from each property in {color} set. {owner} refunded {refund}"
        ...
        with self.dice_lock:
            ...
            if d=="dice4":
                ...
                if self.jailed.get(pl_key, False):
                    c.privmsg(self.channel,f"{pl_key} is in jail! Only !dice4 can be used."); return
