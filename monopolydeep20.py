...
CONSECUTIVE_DOUBLES_FOR_TELEPORT = 2
class MonopolyBot(SingleServerIRCBot):
    def __init__(self, ch, nick, server, port, users=None, num_players=4):
        ...
        self.board_deep = {40:"x-Hospital",41:"x-Serbia",42:"x-Croatia",43:"Japan",44:"x-Austria",45:"x-Italy",46:"x-Internet",47:"x-Belgium",48:"Chest",49:"Spain",50:"x-Chile",51:"x-Argentina",52:"x-Power",53:"x-Brazil",54:"Switch",55:"Korea",56:"x-Indonesia",57:"x-Malaysia",58:"x-Water",59:"x-Singapore",60:"Clover",61:"India",62:"GotoJail",63:"x-Romania"}
        ...
        self.house_costs = {"red":200,"orange":200,"yellow":150,"green":100,"blue":100,"pink":150,"brown":60,"dblue":50,"white":60,"purple":100,"aqua":140,"black":180}
        ...
        self.mortgage_table = {1:25,3:30,4:100,5:100,6:50,8:50,9:60,11:75,12:75,13:100,14:90,15:100,16:100,17:100,19:110,21:110,22:110,24:120,25:100,26:140,27:100,28:140,29:150,31:150,33:150,34:160,35:100,37:180,38:100,39:200,40:75,41:60,42:50,44:50,45:110,46:75,47:120,48:60,50:60,51:90,52:75,53:100,54:70,56:70,57:150,58:75,59:160,60:90,62:90,63:75}
        ...
        self.mortgaged_colors = {"p1":"black","p2":"grey","p3":"white","p4":"lightpink"}
        self.mortgaged2_colors = {"m1":"black","m2":"grey","m3":"white","m4":"lightpink"}
    ...
    def reset_state(self):
        self.players={}; self.num_players=0; self.properties={}; self.houses={}; self.mortgaged=set()
    ...
    def handle_command(self, who, body):
        ...
        if m := re.match(r"!alias\s+(\w+)\s+(\w+)", body):
            pl, a = m.groups(); pl = pl.lower()
            if pl not in self.aliases: return f"{pl} does not exist."
            if any(a.lower() in s for s in self.aliases.values()): return "Alias already used"
            if len(self.aliases[pl]) >= 2: return "Too many aliases"
            self.aliases[pl].add(a.lower()); return f"Alias '{a}' added for {pl}"
        if m := re.match(r"!unalias\s+(\w+)", body):
            a = m.group(1).lower()
            for pl, s in self.aliases.items():
                if a in s: s.remove(a); return f"Alias '{a}' removed from {pl}"
            return f"Alias '{a}' not found"
        if m:=re.match(r"!start\s+(\d+)\s*(deep|regular)?",body):
            ...
            try:
                for cmd in ["!cleardot","!clearall",f"!dotlocation {'1' if mode=='regular' else '2'}"]: self.connection.privmsg("player1bot",cmd)
            except Exception as e: return f"Game started but failed to send initial commands: {e}"
            return f"Game started with {n} players ({mode} mode, positions 0-{self.max_pos})"
        ...
        if m := re.match(r"!teleport\s+(\w+)\s+(-?\d+)", body):
            ...
            return f"{pl_key} teleported to {pos} without paying rent"
        ...
        if m := re.match(r"!switch\s+(\w+)\s+(\w+)", body):
            ...
            return f"{k1} and {k2} switched positions: {k1}->{self.players[k1]['pos']}, {k2}->{self.players[k2]['pos']}"
        if body.lower() == "!propertylist":
            try:
                with open("property.txt","w",encoding="utf-8") as f:
                    f.writelines(f"{p}:{self.active_board[p]}\n" for p in sorted(self.active_board))
                for delay,pos in enumerate(sorted(self.active_board)):
                    ...
                    threading.Timer(delay * 2,lambda p=pos,c=color:self.connection.privmsg("player1bot",f"!dot {p} {c}")).start()
                return "Property list saved and dot markers sent."
        ...
        if body.lower() == "!housestatus":
            ...
            for color, positions in self.color_positions.items():
                total_houses = sum(self.houses.get(pos, 0) for pos in positions)
                result_lines.append(f"{color}:{total_houses}")
                try:
                    if total_houses >= self.hotel_numbers.get(color, 10):
        ...
        if body.lower()=="!status":
            ...
            for p,d in self.players.items():
                ...
                line=f"{p}:${d['money']},pos {d['pos']}|Properties:{props}"
                lines.append(line)
                try:self.connection.privmsg("player1bot",f"!d2 {line}")
                except:pass
            return "|".join(lines)
        if body.lower() == "!up":
            if not self.players or self.num_players == 0: return "No game in progress. Cannot update."
            plist = sorted(self.players.keys())
            pos = [str(self.players[p]["pos"]) for p in plist]
            money = [str(self.players[p]["money"]) for p in plist]
            props = [str(sum(1 for o in self.properties.values() if o == p)) for p in plist]
            houses = [str(sum(self.houses.get(pr,0) for pr, own in self.properties.items() if own == p)) for p in plist]
            while len(pos) < 4:
            ...
            try:
                ...
                return f"Failed to send !mv/!set commands: {e}"
            try:
                msg = " ".join(pos[:self.num_players] + money[:self.num_players] + props[:self.num_players] + houses[:self.num_players])
                self.connection.topic(self.channel, msg)
            except Exception as e:
                return f"Failed to set topic: {e}"
            return None
        m = re.match(r"!auction\s+(\d+)", body)
        if m:
            pos = int(m.group(1)); prop = self.board_regular.get(pos, f"Position {pos}")
            if self.current_auction: return "Auction already in progress."
            if pos in self.non_property or pos in self.properties or pos in self.mortgaged: return f"Cannot auction {prop}."
            self.active_board[pos] = f"x-{prop}"
            self.current_auction = {"pos":pos,"bids":{},"last_bidder":None,"active_players":list(self.players.keys()),"current_index":0}
            try: self.connection.privmsg("player1bot", f"!d2 Auction started for {prop}")
            except: pass
            return f"Auction started for {prop}. {self.current_auction['active_players'][0]}'s turn."
        ...
        if m and self.current_auction:
            ...
            cur = auc["active_players"][auc["current_index"]]
            if player_key != cur: return f"Not your turn. Current turn: {cur}."
            new = max(auc["bids"].values(), default=0) + inc
            if self.players[player_key]["money"] < new: return f"Not enough money for this bid ({new})"
            auc["bids"][player_key] = new; auc["last_bidder"] = player_key
            auc["current_index"] = (auc["current_index"] + 1) % len(auc["active_players"])
            w = max(auc["bids"], key=auc["bids"].get); amt = auc["bids"][w]; prop = self.active_board.get(auc["pos"], f"Position {auc['pos']}")
            msg = f"{w} is winning {prop} with {amt}"
            try:
                
                self.connection.privmsg("player2bot", "!sound bid.mp3")
                self.connection.privmsg("player1bot", f"!d2 {msg}")
            except: pass
            return f"{msg}. Next turn: {auc['active_players'][auc['current_index']]}"
        if body.lower() == "!fold" and self.current_auction:
            ...
            cur = auc["active_players"][auc["current_index"]]
            if player_key != cur: return f"Not your turn. Current turn: {cur}."
            auc["active_players"].remove(player_key)
            if not auc["active_players"] or not auc["bids"]:
                pos = auc["pos"]; prop = self.active_board.get(pos, f"Position {pos}"); self.current_auction = None
                return f"Auction ended. No winner for {prop}."
            if len(auc["active_players"]) == 1:
                winner = auc["active_players"][0]; amt = auc["bids"].get(winner, 0); pos = auc["pos"]
                self.players[winner]["money"] -= amt; self.properties[pos] = winner
                raw = self.active_board.get(pos, f"Position {pos}"); self.active_board[pos] = f"{winner}-{raw[2:] if raw.startswith('x-') else raw}"
                color = self.unmortgaged_colors.get(winner, "red")
                try:
                    self.connection.privmsg(self.channel, f"auction-folded {winner} wins {self.active_board[pos]} for {amt}")
                ...
                self.current_auction = None; return None
            auc["current_index"] %= len(auc["active_players"])
            return f"{player_key} folds. Next turn: {auc['active_players'][auc['current_index']]}"
        if body.lower() == "!resetauction":
            if not self.current_auction: return "No auction in progress to reset"
            auc = self.current_auction; auc["bids"] = {}; auc["last_bidder"] = None
            if "active_players" not in auc: auc["active_players"] = list(self.players.keys()); auc["current_index"] = 0
            elif "current_index" not in auc: auc["current_index"] = 0
            return f"Auction for property {auc['pos']} has been RESET. Bidding restarted."
                        ...
                        if self.current_auction:
                            ...
                            if caller_player != owner:
                                ...
                                if val <= 0:
                                    ...
                                    msg = f"mortgaged {owner} property {pos} for {val-pen} (5% penalty)"
                        ...
                        if self.current_auction:
                            ...
                            if caller_player != owner:
                                ...
                                if self.players[owner]['money'] < cost:
                                    ...
                                    msg = f"unmortgaged {owner} property {pos} for {cost}"
                                    self.msg_queue.put(("player1bot", f"!d2 {msg}"))
        ...
        if m:
            ...
            self.players[owner]['money'] -= cost
            nums = [str(x) for x in props if self.houses.get(x,0) < 5]; hotels = [str(x) for x in props if self.houses.get(x,0) == 5]
        ...
        if m:
            ...
            state = {"players":self.players,"properties":self.properties,"houses":self.houses,
                   "mortgaged":self.mortgaged,"aliases":self.aliases,"current_auction":self.current_auction,
                   "current_trade":self.current_trade,"active_board":self.active_board,
                   "non_property":self.non_property,"num_players":self.num_players,"max_pos":self.max_pos}
            try:
        ...
        if m:
            ...
            try:
                ...
                self.mortgaged = state["mortgaged"]; self.aliases = state["aliases"]; self.current_auction = state["current_auction"]
                ...
                return f"Failed to restore game: {e}"
        ...
        if m:
            ...
            if auc and "active_players" in auc and rm in auc["active_players"]:
                ...
                idx = auc.get("current_index", 0)
                if idx >= len(auc["active_players"]): auc["current_index"] = 0
                if len(auc["active_players"]) == 1:
                    ...
                    try:
                        self.connection.privmsg("player1bot", f"!d2 {winner} wins {pname} for {amt}");
                    except: pass
                    self.current_auction = None
                    return f"{winner} automatically wins {pname} due to player removal"
            return f"{rm} has been removed from the game"
        m = re.match(r"!offer-(p\d+)\s+(.+)", body)
        if m:
            offerer = m.group(1)
            text = m.group(2).strip()
            if offerer not in self.players:
                return f"{offerer} is not a valid player."
            caller_player = next((k for k,s in self.aliases.items() if caller in s or caller==k), None)
            if caller_player != offerer:
                return f"Only {offerer} or their alias can make this offer."
            if self.current_trade:
                return "A trade is already active. !accept or !reject it first."
            parts = text.split()
            other = next((p for p in parts if p.startswith("p") and p != offerer), None)
            if other not in self.players:
                return f"{other} not in game."
            idx = parts.index(other)
            left_tokens = parts[:idx]
            right_tokens = parts[idx+1:]
            ...
            def validate_houses(prop_list, owner):
                for pos in prop_list:
                    if self.houses.get(pos,0) > 0:
                        for color, group in self.color_sets.items():
                            if pos in group:
                                needed = set(group)
                                if not needed.issubset(set(prop_list)):
                                    return f"Property {pos} has houses; must trade entire color set: {group}"
                return None
            err = validate_houses(left_props, offerer)
            if err: return err
            err = validate_houses(right_props, other)
            if err: return err
            for pos in left_props:
                if self.properties.get(pos) != offerer:
                    return f"{offerer} does not own property {pos}"
            for pos in right_props:
                if self.properties.get(pos) != other:
                    return f"{other} does not own property {pos}"
            self.current_trade = {
                "offerer": offerer,
                "other": other,
        ...
        if body.lower() == "!accept":
            ...
            caller_player = next((k for k,s in self.aliases.items() if caller in s or caller==k), None)
            if caller_player != t["other"]:
                return f"Only {t['other']} may !accept this trade."
            if self.players[t["offerer"]]["money"] < t["left_money"]:
                return f"{t['offerer']} does not have enough money."
            if self.players[t["other"]]["money"] < t["right_money"]:
                return f"{t['other']} does not have enough money."
            ...
            def transfer(props, old_owner, new_owner):
                for pos in props:
                    self.properties[pos] = new_owner
                    raw = self.active_board.get(pos, f"Position {pos}")
                    nm = raw.split("-",1)[-1] if "-" in raw else raw
                    self.active_board[pos] = f"{new_owner}-{nm}"
        ...
        if body.lower() == "!reject":
            ...
            t = self.current_trade
            caller_player = next((k for k,s in self.aliases.items() if caller in s or caller==k), None)
            if caller_player not in (t["offerer"], t["other"]):
                return "Only involved players may reject the trade."
            self.current_trade = None
    ...
    def move_player(self,p,sp):
        ...
        if in_reg and old+sp>39: self.players[p]["money"]+=200
        elif not in_reg and old+sp>63: self.players[p]["money"]+=100
        elif not in_reg and old + sp >= 40 and old < 40: self.players[p]["money"] += 100
        self.players[p]["pos"] = new
        name = self.active_board.get(new, f"Position {new}")
        owner = self.properties.get(new)
        msg = ""
        def send_msg(m):
            if m:
                self.connection.privmsg(self.channel, m)
                self.connection.privmsg("player1bot", f"!d2 {m}")
        ...
        if new in rails:
            if new in (5,15,25,35):
                ...
                if not owner and str(self.active_board.get(new,"")).startswith("x-") and not self.current_auction:
                    order = [x for x in self.players if x != p]; order.insert(0, p)
                    self.current_auction = {"pos":new,"bids":{},"last_bidder":None,"active_players":order,"current_index":0}
                    raw = self.active_board.get(new,""); msg += f" | Auction automatically started for {raw[2:] if raw.startswith('x-') else raw}. Start bidding"
                    ...
                    if new in self.mortgaged: msg += " | Property is mortgaged, no rent"
                        ...
                        rc = [x for x in (5,15,25,35) if self.properties.get(x) == owner and x not in self.mortgaged]; rent = {1:25,2:50,3:100,4:200}.get(len(rc),25)
                        self.players[p]["money"] -= rent; self.players[owner]["money"] += rent; play_rent_sound(); msg += f" | {p} pays {rent} to {owner} (railroad)"
            if self.max_pos > 39:
                target = rails[new]; self.players[p]["pos"] = target; new = target; name = self.active_board.get(new, f"Position {new}"); msg += f" | Teleported via railroad to {new}"
                owner = self.properties.get(new)
        ...
        total_houses = sum(
            h for pos, h in self.houses.items()
            if self.properties.get(pos) == p
        )
        total_props = sum(1 for o in self.properties.values() if o == p)
        
        if new in (2,18,36,48): self.players[p]["money"] += 50; msg += " +50"; self.connection.privmsg("player2bot", "!sound bonus.mp3")
        elif new in (7,60): fee = total_houses * 5 + total_props * 5; self.players[p]["money"] -= fee; msg += f" pays {fee}"; self.connection.privmsg("player2bot", "!sound tax.mp3")
        elif new == 23: self.players[p]["money"] -= (fee := total_houses*10 + max(0,int(self.players[p]["money"]*0.05))); msg += f" pays {fee}"; self.connection.privmsg("player2bot","!sound tax.mp3")

        elif new == 32: fee = total_props * 10; self.players[p]["money"] -= fee; msg += f" pays {fee}"; self.connection.privmsg("player2bot", "!sound tax.mp3")
        elif new in (30,62): 
            self.jailed[p] = True
            ...
            return "Jail", msg
        if owner and owner != p:
            if new in self.mortgaged: 
                msg += " | Property is mortgaged, no rent"
            elif self.jailed.get(owner, False): 
                msg += " | owner in jail, no rent"
            else:
                if new in (4,13,27,38,40,46,52,58):
                    u = [x for x in (4,13,27,38,40,46,52,58) if self.properties.get(x) == owner and x not in self.mortgaged]
                    mult = {1:4,2:8,3:15,4:25,5:35,6:45,7:55,8:75}.get(len(u),4)
                    rent = abs(sp) * mult
                    self.players[p]["money"] -= rent
                    self.players[owner]["money"] += rent
                    play_rent_sound()
                    msg += f" | Utility {sp}*{mult}={rent}"
                else:
                    ...
                    for color,props in self.color_sets.items():
                        if new in props: 
                            if all(self.properties.get(x)==owner for x in props) and all(self.houses.get(x,0)==0 for x in props): 
                                rent*=2
                            break
        ...
        if new not in self.non_property and str(self.active_board.get(new,"")).startswith("x-") and new not in self.properties and not self.current_auction and new not in (43,49,55,61):
            order = [x for x in self.players if x != p]; order.insert(0, p)
            self.current_auction = {"pos":new,"bids":{},"last_bidder":None,"active_players":order,"current_index":0}
            raw = self.active_board.get(new,""); msg += f" | Auction automatically started for {raw[2:] if raw.startswith('x-') else raw}."
    ...
    def override_turn(self, c, nick, msg):
        if msg=='!gooverride':
            ...
            c.privmsg(self.channel, f"{nick} used GO override! Next turn is open to any player")
    def handle_go_command(self, c, msg, nick):
        if m := re.match(r"!go([1-4])(?:\s+(\d+))?", msg):
            if not m: return
            if self.players and self.any_player_negative():
                c.privmsg(self.channel, "GO commands disabled: a player has negative balance.")
                return
            if not self.go_enabled: c.privmsg(self.channel, f"{nick}, you need to start a GO session first with !gostart"); return
            cmd = m.group(1)
            if nick in self.aliases.get('p1', []) or nick == 'p1': pl_key='p1'
            elif nick in self.aliases.get('p2', []) or nick == 'p2': pl_key='p2'
            else: c.privmsg(self.channel, f"{nick}, you are not allowed to use GO commands."); return
            if cmd in ('1','2') and self.jailed.get(pl_key, False) and not self.go_jail_key.get(pl_key, False):
                c.privmsg(self.channel, f"{nick}, !go{cmd} cannot be used until !gojailkey is used.")
                return
            if pl_key=='p1' and cmd not in ('1','3'): c.privmsg(self.channel, f"{nick}, only !go1 or !go3 allowed"); return
            if pl_key=='p2' and cmd not in ('2','4'): c.privmsg(self.channel, f"{nick}, only !go2 or !go4 allowed"); return

            if cmd in ('3', '4') and not self.jailed.get(pl_key, False):
                c.privmsg(self.channel, f"!go{cmd} can only be used when player is in jail.")
                return
                
            if not self.override_next_turn and self.turn != pl_key: c.privmsg(self.channel, f"{nick}, not your turn. Use !gooverride."); return
            self.override_next_turn=False
            self.go_owner = pl_key
            self.start_go(c, m)
    def handle_gojailkey(self, c, msg):
        if not msg.lower().startswith("!gojailkey"):
            return
        m = re.match(r"!gojailkey\s+(p[1-2])", msg.lower())
        if not m:
            c.privmsg(self.channel, "Usage: !gojailkey p<number> (!gojailkey p1/p2)")
            return
        pl_key = m.group(1)
        if not self.jailed.get(pl_key, False):
            c.privmsg(self.channel, f"{pl_key} is not in jail.")
            return
        self.go_jail_key[pl_key] = True
        c.privmsg(self.channel, f"{pl_key} has used !gojailkey. Now !go1/!go2 can be used.")
        try: c.privmsg("player2bot", "!sound key.mp3")
        except: pass

    def handle_go_privmsg(self, c, user, msg):
        if not self.go_active or user not in self.go_input_users:
            return
        if not msg.isdigit() or not (0 <= int(msg) <= 7):
            c.privmsg(user, "number must be 0-7")
            return
        ...
        if user.lower() in ("player1bot", "player2bot"):
            sound = "click.mp3" if user.lower() == "player1bot" else "dice.mp3"
            try: c.privmsg("player2bot", f"!sound {sound}")
            except: pass
        if len(self.go_numbers) == len(self.go_input_users):
            self.end_go(c, "completed")
                
    def start_go(self, c, m):
        if not self.go_lock.acquire(blocking=False):
            c.privmsg(self.channel, "Another GO is already active"); return
        self.go_active, self.go_numbers = m.group(1), {}
        timeout = int(m.group(2)) if m.group(2) else 60
        c.privmsg(self.channel,f"!go{self.go_active} started. Waiting to receive (1-6) number privately from P1 and P2. Timeout: {timeout}s")
        if self.go_timer: self.go_timer.cancel()
        self.go_timer = threading.Timer(timeout, self.timeout, [c]); self.go_timer.start()
    def timeout(self, c):
        missing=[u for u in self.go_input_users if u not in self.go_numbers]
        if missing: c.privmsg(self.channel,f"!go{self.go_active} timed out. Missing input from: {', '.join(missing)}")
        self.end_go(c,"timed out")
    def end_go(self, c, reason):
        # Collect numbers (default 0 if missing)
        ...
        if reason == "completed" and len(self.go_numbers) == len(self.go_input_users):
            # Determine acting player from GO command
            player = 'p1' if self.go_active in ('1', '3') else 'p2'
            jail_go = self.go_active in ('3', '4')
            # Jail GO requires doubles to move
            if jail_go and not double:
                c.privmsg(
                    self.channel,
                    f"!go{self.go_active} requires doubles to move. "
                    f"Dice: {nums[0]} and {nums[1]}"
                )
                # Turn always passes on failed jail GO
                self.turn = 'p2' if player == 'p1' else 'p1'
                c.privmsg(self.channel, f"Turn switched to {self.turn}")
            else:
                # Perform movement
                self.handle_command("dicebot", f"!move {player} {total}")
                c.privmsg(self.channel, f"Dice results: {nums[0]} and {nums[1]}")
                # Announce double / next turn
                pm = {
                    True: {
    ...
    def _handle_dicestart(self,c,m):
        ...
        with self.dice_lock:self.dice_mode=True;self.dice_players=n;self.dice_order=list(range(1,n+1));self.expected_player_index=0;self.dice_override=False;self.disabled_dice.clear()
        c.privmsg(self.channel,f"Dice mode started for {n} players. First roll: !diceX-p1")
