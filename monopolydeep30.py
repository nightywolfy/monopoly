...
class MonopolyBot(SingleServerIRCBot):
    def __init__(self, ch, nick, server, port, users=None, num_players=4):
        ...
        self.house_costs = {"red":200,"orange":180,"yellow":150,"green":100,"blue":100,"pink":150,"brown":60,"dblue":60,"white":50,"purple":80,"aqua":100,"black":120}
    ...
    def handle_command(self,who,body):
        ...
        if m:=re.match(r"!passgo\s+(p\d+)\s+(\d+)\s+(\d+)",body):
        ...
        if body.lower()=="!fold" and self.current_auction:
            ...
            if player_key==highest_bidder:
                return f"{player_key} is currently highest bidder"
    ...
    def move_player(self,p,sp):
        ...
        bonus_data=self.passgo_bonus.get(p)
        ...
        if new in (2,18,36,48):
            ...
            import random
            tp=random.choice([x for x in range(40,64) if x not in (43,49,54,55,61,62)])
            self.players[p]["pos"]=tp;msg+=f" teleported to {tp}"
            self.connection.privmsg(self.channel,f"{p} teleported to {tp}")
            self.connection.privmsg("player1bot",f"!teleport {p} {tp}")
            return self.active_board.get(tp,f"Position {tp}"),msg

        if new==54:self.switch_required=True;msg+=" | Must use !switch before next dice roll"
        if new==20:self.auction_required=True;msg+=" | Auction required before next dice roll"
        ...
        if owner and owner!=p:
            if new in self.mortgaged:msg+=" | Property is mortgaged, no rent"
                ...
                if new in (4,13,27,38):
                    ...
                    mult={1:4,2:8,3:12,4:25}.get(len(u),4);rent=abs(sp)*mult
