...
class MonopolyBot(SingleServerIRCBot):
    ...
    def handle_command(self, who, body):
        ...
        if body.lower() == "!housestatus":
            ...
            for color, positions in self.color_positions.items():
                total_houses = sum(self.houses.get(pos, 0) for pos in self.color_sets.get(color, positions))
                result_lines.append(f"{color}:{total_houses}")
                try:
                    if all(self.houses.get(pos, 0) >= 5 for pos in self.color_sets.get(color, positions)):
    ...
    def move_player(self,p,sp):
        ...
        def send_msg(m):
            if m: self.connection.privmsg(self.channel,m); self.connection.privmsg("player1bot",f"!d2 {m}")
        ...
        if new in (2,18,36,48):
            ...
            fee=total_houses*10+max(0,int(self.players[p]["money"]*0.05)); self.players[p]["money"]-=fee; msg+=f" pays {fee}"; self.connection.privmsg("player2bot","!sound tax.mp3")
        elif new==32:
            fee=total_props*10; self.players[p]["money"]-=fee; msg+=f" pays {fee}"; self.connection.privmsg("player2bot","!sound tax.mp3")
        elif new in (30,62):
        ...
        if owner and owner!=p:
            if new in self.mortgaged: msg+=" | Property is mortgaged, no rent"
                ...
                if new in (4,13,27,38):
                    ...
                    mult={1:4,2:8,3:15,4:30}.get(len(u),4); rent=abs(sp)*mult
                    ...
                    mult={1:4,2:8,3:15,4:30}.get(len(u),5); rent=abs(sp)*mult
                    self.players[p]["money"]-=rent; self.players[owner]["money"]+=rent; play_rent_sound(); msg+=f" | Utility {sp}*{mult}={rent}"
                else:
