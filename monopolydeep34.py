...
class MonopolyBot(SingleServerIRCBot):
    ...
    def _handle_diceadd(self,c,pn):
        with self.dice_lock:
            ...
            current_player=self.dice_order[self.expected_player_index] if self.dice_order else None
            self.dice_order.append(pn);self.dice_order.sort()
            if current_player is not None:self.expected_player_index=self.dice_order.index(current_player)
